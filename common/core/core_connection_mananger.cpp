...
void Connection_Manager::get_address_pair(const Server_Node_t& node, Inet_Addr& src_addr, Inet_Addr& dst_addr) {
    ...
    if (local_type == 0x00ff) { //本地是双线,
        ...
        // check intranet flags
        uint16_t src_type = local_type & 0x00F0;
        uint16_t dest_type = remote_type & 0x00F0;

        if (src_type == 0x00E0 || dest_type == 0x00E0) {
            if (src_type != dest_type) {
                CORE_WARNING("only one peer has intranet addr in src addr and dest addr, try public ip");
...
void Connection_Manager::connecting_by_id(const string& data, uint32_t sid) {
    ...
    if (it != nodes_.end()) {
        ...
        if (conn->get_state() != CCoreConnection::CONN_CONNECTED) //处于连接状态，缓冲到LIST中
...
CCoreConnection* Connection_Manager::get_connection(uint32_t sid) {
    ...
    if (it != nodes_.end() && it->second.conn != NULL
            && it->second.conn->get_state() == CCoreConnection::CONN_CONNECTED) { //只获取连接完成的CONNECTION,否则返回1个NULL
        ret = it->second.conn;
    }
