...
{
	string ret = "";
	ret.resize(in_size * 2);

	void *pdata = (void*)ret.data();
	uint8_t* pos = (uint8_t *)pdata;

	for(int32_t i = 0; i < in_size; ++i)
	{
		*pos = asc_map[in[i]][0];
		*(pos + 1) = asc_map[in[i]][1];
...
void asc2bin(const string& in, uint8_t* out, int32_t out_size, int32_t &out_length)
{
	...
	void *pdata = (void*)in.data();
	uint8_t* pos = (uint8_t *)pdata;

	for(int32_t i = 0; i < out_length; ++ i)
	{
		uint8_t h = bin_map[*pos];
		uint8_t l = bin_map[*(pos + 1)];
