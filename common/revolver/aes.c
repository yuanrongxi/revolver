...
void aes_gen_tables( void )
{
    ...
    for( i = 0; i < 256; i++ )
    {
        ...
        RT0[i] = ( (unsigned int) MUL( 0x0B, y )       ) ^
                 ( (unsigned int) MUL( 0x0D, y ) <<  8 ) ^
                 ( (unsigned int) MUL( 0x09, y ) << 16 ) ^
                 ( (unsigned int) MUL( 0x0E, y ) << 24 );
...
void aes_encrypt( aes_context *ctx, const unsigned char input[16], unsigned char output[16] )
{
...
void aes_decrypt( aes_context *ctx,const  unsigned char input[16], unsigned char output[16] )
{
    unsigned int *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
