...
void RUDPCCCObject::on_ack(uint64_t ack_seq) {
    ...
    if (slow_start_) { //慢启动窗口决策
        ...
        if (recv_count_ / 2 < resend_count_/*snd_cwnd_ * rtt_var_ * 5 / (rtt_ * 8) || resend_count_ > rtt_ / 6*/) //出现丢包, 不做加速
            loss_flag_ = false;
...
void RUDPCCCObject::on_timer(uint64_t now_ts) {
    uint32_t delay = core_max(rtt_var_ * 10, 50);
    if (now_ts >= prev_on_ts_ + delay) { //10个RTT决策一次
        print_count_ ++;
        if (print_count_ % 100 == 0) {
            RUDP_DEBUG("send window size = " << snd_cwnd_ << ",rtt = " << rtt_ << ",rtt_var = "
                       << rtt_var_ << ",resend = " << resend_count_);
            set_max_cwnd(rtt_);
        }
