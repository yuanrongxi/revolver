...
void RUDPObject::set_userdata(int32_t rudp_id, void* data)
{
    RUDPSocket *sock = get_socket(rudp_id);
    if (sock)
        sock->set_userdata(data);
}

void* RUDPObject::get_userdata(int32_t rudp_id)
{
    void* ret = 0;
    RUDPSocket *sock = get_socket(rudp_id);
    if (sock)
        ret = sock->get_userdata();
